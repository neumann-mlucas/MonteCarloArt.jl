#!/usr/bin/env bash
# Task 1 (color-aware acceptance) — same corpus as baseline, sweeps color-tolerance.

HERE="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" &>/dev/null && pwd)"
PROJECT_DIR="$(cd -- "${HERE}/../.." &>/dev/null && pwd)"
CORPUS_DIR="${PROJECT_DIR}/test/fixtures"
OUT_DIR="${PROJECT_DIR}/test/results/task1"
NORM_DIR="${OUT_DIR}/_inputs"
BENCH_CSV="${OUT_DIR}/bench.csv"
JULIA_BIN="${JULIA_BIN:-julia}"
JULIA_OPTS=(-O3 -t 8)
CSV_HEADER="image,config,steps,palette,tol,color_tol,seconds,circles,status,git_sha"

# name|steps|palette|tol|color_tol
CONFIGS=(
  "fast|100000|64|0.08|0.0"
  "full|300000|64|0.08|0.0"
)
...
run_config() {
  ...
  IFS='|' read -r name steps palette tol ctol <<< "$cfg"
  ...
  echo "${img},${name},${steps},${palette},${tol},${ctol},${LAST_SECS},${circles:-},${LAST_STATUS},$(git_sha)" >> "$BENCH_CSV"
